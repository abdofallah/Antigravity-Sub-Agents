 ...
 *
 * The body of the IIFE is wrapped in a router-navigation lock:
 *   window.__saIsInjecting = true; try { ... } finally { release }
 * which coordinates with the navigation blocker installed by
 * build-router-sub.ts so that route changes pause until in-flight DOM
 * work completes — preventing stale closures from previous routes from
 * polluting a freshly-loaded conversation.
 *
...
export interface PanelInjectionData {
    ...
    /**
     * The conversation ID the Node-side believes is currently active.
     * Used as a strict guard inside the browser script: if the router-derived
     * activeConvoId doesn't match this, we abort without touching the DOM.
     */
    expectedConvoId?: string;
    /**
     * Monotonic route-generation counter from Node side. Stamped into the
     * payload so out-of-sync browser evaluations can be diagnosed.
     */
    routeGeneration?: number;
    /**
     * Strictly-monotonic injection sequence number (Node-side counter).
     * The browser tracks the last-seen seq per convo and rejects any
     * evaluation whose seq is less than the last successful render for
     * the same convo. This eliminates out-of-order CDP evaluations.
     */
    dataSequence?: number;
}
...
export function buildPanelScript(data: PanelInjectionData): string {
    const { prefix, agents, visibleLimit, dataHash, subAgentIds, pendingActions, parentMap, parentTitles, uiPollInterval, debugLogging, expectedConvoId, routeGeneration, dataSequence } = data;
    const dataJson = JSON.stringify(agents);
    const expectedConvoIdJson = JSON.stringify(expectedConvoId || '');
    const routeGenJson = JSON.stringify(routeGeneration ?? 0);
    const dataSeqJson = JSON.stringify(dataSequence ?? 0);
    ...
    return `(() => {
            // ── Lock router navigation while we do heavy DOM work ──
            // Any user-initiated route change is paused (via the blocker
            // installed in build-router-sub.ts) until the finally{} block
            // below calls __saReleaseNavigation. This is what prevents
            // stale closures from a previous route bleeding into the new one.
            window.__saIsInjecting = true;
            try {
                try {
                    var P = '${prefix}';
                    ...
                    // ── Node-side expectations (strict guard) ──
                    var expectedConvoId = ${expectedConvoIdJson};
                    var routeGeneration = ${routeGenJson};
                    debug.expectedConvoId = expectedConvoId ? expectedConvoId.substring(0, 8) : 'none';
                    debug.routeGeneration = routeGeneration;
                    ...
                    // ══ CRITICAL: Route-coherence guard ══
                    // If Node thinks we're on convo X but the browser router still says
                    // 'none' (or a different convo), the page is mid-transition. Abort
                    // BEFORE any DOM mutations so we don't wipe a freshly-rendered panel.
                    //
                    // Three abort cases (all preserve existing DOM):
                    //   1. expectedConvoId empty → Node-side believes non-convo route.
                    //      A graduated-retry timer could still land here; bail out.
                    //   2. activeConvoId empty → browser router hasn't resolved yet.
                    //   3. activeConvoId ≠ expectedConvoId → mid-navigation race.
                    if (!expectedConvoId) {
                        _saLog('GUARD: expectedConvoId empty (Node-side non-convo route) — abort, no DOM mutations');
                        debug.guard = 'no-expected';
                        debug.trace = _saTrace;
                        return JSON.stringify({ ok: true, state: 'guard-skip', sections: { chatbox: 'guard', watcher: 'guard', sidebar: 'guard' }, count: 0, activeCount: 0, debug: debug });
                    }
                    if (!activeConvoId) {
                        _saLog('GUARD: router activeConvoId empty (expected=' + expectedConvoId.substring(0,8) + ') — abort, preserve DOM');
                        debug.guard = 'router-empty';
                        debug.trace = _saTrace;
                        return JSON.stringify({ ok: true, state: 'guard-skip', sections: { chatbox: 'guard', watcher: 'guard', sidebar: 'guard' }, count: 0, activeCount: 0, debug: debug });
                    }
                    if (activeConvoId !== expectedConvoId) {
                        _saLog('GUARD: convo mismatch (browser=' + activeConvoId.substring(0,8) + ', expected=' + expectedConvoId.substring(0,8) + ') — abort, preserve DOM');
                        debug.guard = 'convo-mismatch';
                        debug.trace = _saTrace;
                        return JSON.stringify({ ok: true, state: 'guard-skip', sections: { chatbox: 'guard', watcher: 'guard', sidebar: 'guard' }, count: 0, activeCount: 0, debug: debug });
                    }

                    // ══ DEFENSE 1: Monotonic sequence guard (per-convo) ══
                    // Node stamps every injection with a strictly-increasing dataSequence.
                    // If a CDP Runtime.evaluate response we sent earlier arrives AFTER a
                    // later one (out-of-order due to V8 scheduling, microtask interleaving,
                    // or queued promises), reject it. Otherwise the older payload (often
                    // built from a stale orchestrator snapshot) would wipe the panel.
                    var dataSequence = ${dataSeqJson};
                    debug.dataSequence = dataSequence;
                    if (!window.__saLastRender) window.__saLastRender = {};
                    var lastRender = window.__saLastRender[activeConvoId];
                    if (lastRender && typeof lastRender.seq === 'number' && dataSequence > 0 && dataSequence < lastRender.seq) {
                        _saLog('GUARD: out-of-order seq (incoming=' + dataSequence + ' < last=' + lastRender.seq + ') — abort, preserve DOM');
                        debug.guard = 'stale-seq';
                        debug.trace = _saTrace;
                        return JSON.stringify({ ok: true, state: 'guard-skip', sections: { chatbox: 'guard', watcher: 'guard', sidebar: 'guard' }, count: 0, activeCount: 0, debug: debug });
                    }
                    ...
                    // ══ DEFENSE 2: Anti-wipe shrink guard ══
                    // The agent universe (allAgents) should only ever GROW within a session
                    // (new sub-agents launched). It NEVER shrinks except via clearHistory.
                    // If this payload claims activeCount=0 but the last successful render
                    // for this convo had active agents AND allAgents has shrunk vs last
                    // render, we are looking at provably stale data. Abort.
                    if (lastRender && activeCount === 0 && lastRender.activeCount > 0 && allAgents.length < lastRender.allAgentsLen) {
                        _saLog('GUARD: shrink-wipe detected (now active=0 was=' + lastRender.activeCount + ', allAgents ' + allAgents.length + '<' + lastRender.allAgentsLen + ', seq=' + dataSequence + ' last=' + lastRender.seq + ') — abort, preserve DOM');
                        debug.guard = 'shrink-wipe';
                        debug.trace = _saTrace;
                        return JSON.stringify({ ok: true, state: 'guard-skip', sections: { chatbox: 'guard', watcher: 'guard', sidebar: 'guard' }, count: 0, activeCount: 0, debug: debug });
                    }

                    // All guards passed — commit hash and remember this render
                    uiState._dataHash = fullHash;
                    window.__saLastRender[activeConvoId] = {
                        seq: dataSequence,
                        activeCount: activeCount,
                        totalCount: totalCount,
                        allAgentsLen: allAgents.length,
                        ts: Date.now()
                    };
...
${lockWatcherFragment}
                    ...
                    if (itemsList && totalCount > 0) {
                        agents.forEach(function(a, idx) {
                            ...
                            if (a.status === 'waiting_for_action' && a.pendingAction) {
                                ...
                                approveBtn.style.cssText = 'padding:1px 6px;background:var(--primary,#2563eb);color:var(--primary-foreground,#fff);border:none;border-radius:4px;font-size:0.75rem;cursor:pointer;transition:opacity 0.12s;z-index: 2;';
                                ...
                                denyBtn.style.cssText = 'padding:1px 6px;background:var(--secondary,rgba(255,255,255,0.08));color:var(--secondary-foreground,rgba(255,255,255,0.7));border:none;border-radius:4px;font-size:0.75rem;cursor:pointer;transition:opacity 0.12s;z-index: 2;';
                            ...
                            if (a.isActive) {
                                var sOverlay = el('div', '');
                                sOverlay.style.cssText = 'position:absolute;top:0;bottom:0;right:0;padding-left:12px;display:flex;align-items:flex-start;justify-content:flex-end;gap:1px;transform:translateX(6px);visibility:hidden;padding-top:2px;background:linear-gradient(to right, transparent 0%, var(--content,rgba(18,18,18,0.95)) 85%);width:100%;z-index:1;';
                                var sBtn = el('button', '');
                                sBtn.style.cssText = 'width:24px;height:24px;display:flex;align-items:center;justify-content:center;border-radius:6px;border:none;background:none;cursor:pointer;padding:0;margin-right:10px;z-index:2;';
                ...
                // ── Release the router navigation lock ──
                // Whether the script succeeded, returned early, or threw, we MUST
                // unlock the router. Otherwise the app gets stuck unable to navigate.
                window.__saIsInjecting = false;
                if (typeof window.__saReleaseNavigation === 'function') {
                    try { window.__saReleaseNavigation(); } catch(_) {}
                }
            }
        })()`;
}
